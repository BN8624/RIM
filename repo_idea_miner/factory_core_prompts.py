...
_CORE_CONTRACT_DRAFT_SCHEMA = {
    "core_contract": {
        ...
        "output_representation": {
            "event_item_type": "object | string",
            "event_required_keys": ["object일 때 모든 이벤트에 반드시 있는 키 (예: type, target_id)"],
            "event_kind_key": "이벤트 종류가 담기는 키 이름 (string이면 항목 자체)",
            "event_kinds": ["허용 이벤트 종류 전부 — actions[].output의 이벤트 이름과 일치"],
            "summary_format": "summary 문자열의 구체적 형식 예 (예: '2 executed, 1 rejected')",
            "summary_rule": "summary가 final_state/errors에서 어떻게 계산되는지 한 문장",
        },
        "forbidden_shortcuts": [
            "hardcoded output",
            "static UI only",
...
- output_representation은 구현과 정답지(golden)가 공유할 표현 계약이다. 반드시 구체적으로 선언하라:
  ...
  summary_rule(final_state/errors에서 계산되는 규칙). summary는 항상 문자열이다 —
  서술형 산문이 아니라 상태에서 기계적으로 계산 가능한 형식이어야 한다.
...
- output_representation이 이벤트/summary 표현을 구체적으로 선언하는가
  (event_kinds가 actions의 output과 일치하는가, summary_format이 계산 가능한 형식인가)
- Phase 2에서 확장 가능한가
...
- **표현 계약 (필수)**: expected_events의 각 항목은 core_contract.output_representation의
  event_item_type/event_required_keys/event_kinds 형식 그대로 써라.
  object로 선언됐는데 "NAVIGATED" 같은 추상 라벨 문자열을 쓰면 lint FAIL이다.
- expected_summary는 output_representation.summary_format을 따르는 계산 가능한 문자열로 써라.
  "성공적으로 이동함" 같은 서술형 산문 요약은 금지다 — 구현이 그 문장을 계산할 수 없다.
...
- 자동 gate로 사용 가능한가
- expected_events/expected_summary가 contract의 output_representation 형식과 일치하는가
  (추상 라벨 이벤트, 서술형 산문 summary는 구현과 비교 불가 → NEEDS_REPAIR)
...
- events/summary는 Core Contract의 output_representation 형식 그대로 출력 (golden과 동일 표현)
...
- events의 각 항목과 summary는 core contract의 output_representation 형식을 정확히 따라라.
  summary는 서술형 문장이 아니라 summary_rule대로 상태에서 계산한 문자열이어야 한다.
...
_MOCK_CORE_CONTRACT = {
    ...
    "output_representation": {
        ...
        "event_kind_key": "type",
        "event_kinds": ["command_executed", "follow_up_executed"],
        "summary_format": "2 cards, 0 rejected",
        "summary_rule": "history_count와 errors 수로 '<N> cards, <M> rejected'를 계산",
    },
    "forbidden_shortcuts": [
        "hardcoded output",
        "static UI only",
