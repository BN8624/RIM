...
- **Phase 2D-1 Closed Product Loop 완료** — `factory-product-loop --execute`가 judge→lane 선택→child run 실행→재검증 루프를 돈다.
  base run은 항상 무손상(§13), gate는 매 verify마다 temp copy에서 재실행(§7), 예산 §10(iter 4 / lane 2 / high-risk 1 / 무진전 2).
...
3. lane 실패에서 발견된 requires_spec_repair를 다음 iteration judge에 전달하는 escalation은 **계획 밖** —
   원하면 주문서 갱신부터(§10 high-risk 예산 1과 함께 설계 필요).
4. flaky 근본 원인 조사 (task chip: patch_attempts=2).

## 4. 중요 결정 (변경하지 말 것)

- closed loop은 원본 base run을 절대 수정하지 않는다(§13) — apply류 옵션 자체가 없음.
- gate/probe는 기록 report를 재사용하지 않고 매번 재실행(§7 — 관측이 서술을 이긴다).
- hard fact rung의 gap/lane은 deterministic (live desk 판정 override — ddb9431). soft rung(viewer/interaction/UX)은 Gemma 판정 존중.
- summary는 하네스 표준상 **항상 문자열**. spec repair §8 보호 우회 자동화 금지.
...
- 다수 run 배치 자동화 미착수 — §16: 두 도메인이 같은 orchestrator를 통과하기 전에는 시작하지 않기로 했고, 이제 통과함(둘 다 정직 HOLD).
